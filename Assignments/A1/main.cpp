...
void repeatrun(unsigned int reps, Data &D, const Ranges &R,
               unsigned int numt) { // Repeat the same experiment. Time each
                                    // rep. D, R, and numt remain fixed.
  int it;
  ...
  std::cout << reps << " iterations of " << D.ndata << " items in " << R.num()
            << " ranges with " << numt << " threads: Fastest took " << minms
            << " ms, Average was " << tms / reps << " ms\n";
}
...
Data readData(const char *filename, unsigned int nItems) {
  ...
  if (d < nItems)
    D.ndata = d - 1;
  fs.close();
  std::random_shuffle(D.data, D.data + D.ndata);
  return D;
}
