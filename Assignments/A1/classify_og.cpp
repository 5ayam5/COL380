...
#pragma omp parallel num_threads(numt)
  {
    ...
    for (int i = tid; i < D.ndata;
         i += numt) { // Threads together share-loop through all of Data
...
#pragma omp parallel num_threads(numt)
  {
    ...
    for (int r = tid; r < R.num();
         r += numt) { // Thread together share-loop through the intervals
      int rcount = 0;
